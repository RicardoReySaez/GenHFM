data {
    ...
    matrix<lower=0>[J,3] pr_scale_tau;
...
parameters {
    ...
    vector<lower=0>[J] shift_tau;       // Shift parameter for inverse of exponential rate
    ...
    vector<lower=0>[J] scale_tau;       // Scale parameter for inverse of exponential rate
    ...
    matrix<lower=0>[J, I] tau_tilde;    // Inverse of exponential rate
}
...
transformed parameters {
    ...
    matrix[I, J] rate  = inv(rep_matrix(shift_tau', I)  + diag_pre_multiply(scale_tau, tau_tilde)');
}
...
model {
    ...
    shift_tau   ~ normal(pr_shift_tau[,1],   pr_shift_tau[,2]);
    ...
    scale_tau   ~ student_t(pr_scale_tau[,1],   pr_scale_tau[,2],   pr_scale_tau[,3]);
    ...
    to_vector(tau_tilde)   ~ std_normal();
...
generated quantities {
    ...
    vector[J] mu_tau = shift_tau + scale_tau .* sqrt(2.0/pi());
    vector[J] sd_tau = scale_tau .* sqrt(1 - 2.0/pi());
    // Signal-to-noise ratios
    vector[J] gamma2 = square(sd_beta)./(square(mu_sigma) + square(sd_sigma) + square(mu_tau) + square(sd_tau));
