...
parameters {
    ...
    real <lower=0> shift_lambda;        // Shift parameter for exponential rate
    ...
    real <lower=0> scale_lambda;        // Scale parameter for exponential rate
    ...
    // Individual-level exponential rate and residual std dev parameter
...
transformed parameters{
    ...
    vector [I] Lambda = shift_lambda + scale_lambda * Lambda_tilde;
...
generated quantities {
    ...
    // Shift and scale parameters for exponential rate in log scale
    real logshift_lambda = log(shift_lambda);
    real logscale_lambda = log(scale_lambda);
    ...
    // Expected value and variance for exponential rate parameter
    real mu_lambda = shift_lambda + scale_lambda * sqrt(2.0/pi());
    real sd_lambda = square(scale_lambda) * (1 - 2.0/pi());
    ...
    // Expected value and variance for exponential rate parameter (log scale)
    real logmu_lambda = log(mu_lambda);
    real logsd_lambda = log(sd_lambda);
    // Tau-square expected value
    real mu_tau2; int B = 5000;
    {
      vector [B] z_vals = abs(to_vector(normal_rng(rep_vector(0,B), rep_vector(1,B))));
      mu_tau2 = mean(inv_square(shift_lambda + scale_lambda * z_vals)); 
    }
    // Signal-to-noise ratios
    real gamma2 = square(sd_theta)/(square(mu_sigma) + square(sd_sigma) + mu_tau2);
